...
Specifically:
Is there variation in what/how much is being funded? 
- Most imporverished areas
- What is being funded/partially funded/not funded and likelihood?
- Female vs. male borrowers and whether having a male in the group effects loan behavior?
...
kiva_loan.join <- kiva_loan1%>%dplyr::select(loan_amount,country)%>%group_by(country)%>%summarise(sumloan_amount=sum(loan_amount))
kiva_loan.join.d <- distinct(kiva_loan.join)

# to join sector and activity with world_region
kiva_loan.join.s <- kiva_loan1%>%dplyr::select(loan_amount,country,sector,activity)
kiva_loan.join.s1 <- distinct(kiva_loan.join.s)
kiva_loan.join.s.d <- kiva_loan.join.s1%>%group_by(country,sector)%>%mutate(loan_amount.sector=sum(loan_amount))%>%group_by(country,activity)%>%mutate(loan_amount.activity=sum(loan_amount))

head(kiva_loan1)
sum(kiva_loan1$funded_amount)-sum(kiva_loan1$loan_amount)
...
mpi.loan.join.s1 <- mpi.loan.join.s%>%dplyr::select(country,world_region,sector,activity,loan_amount.sector,loan_amount.activity)
...
mpi.country.join <- mpi.loan.join1%>%dplyr::select(country,MPI_country,sumloan_amount)
mpi.country.join.d <- distinct(mpi.country.join)

# World Region + Country + Mean MPI by Country + Loan by Country
mpi.loan.country.region <- mpi.loan.join1%>%dplyr::select(world_region,country,MPI_country,sumloan_amount)
mpi.loan.country.region.d <- distinct(mpi.loan.country.region)

#summarize loan and MPI by region
mpi.world_region.join <- mpi.loan.join1%>%dplyr::select(country,world_region,sumloan_amount,MPI_country)
mpi.world_region.d <- distinct(mpi.world_region.join)
mpi.world_region.d1 <- mpi.world_region.d%>%group_by(world_region)%>%mutate(sumloan_amount.region=sum(sumloan_amount),MPI_region=mean(MPI_country))
mpi.world_region.d2 <- mpi.world_region.d1%>%dplyr::select(world_region,sumloan_amount.region,MPI_region)
...
funded <- kiva_loan1%>%filter(funded_amount>0)%>%dplyr::select(country,funded_amount)
# log transform
funded$log.funded <- log(funded$funded_amount)

ggplot(data=funded, aes(x=log.funded))+geom_histogram(col="black",fill="seagreen3")+theme(plot.title=element_text(hjust =0.5,size=19))+labs(title = "Distribution of Funded Loans (Log Scale)", y="Count",x="exp(Loan Amount)")+scale_x_continuous(breaks=c(3,5,7,11), labels=c(format(exp(3),digits=3),format(exp(5),digits=3),format(exp(7),digits=3),format(exp(11),digits=3)))
```

From the histogram, the funded Kiva loan amounts in the dataset range from about 20 USD to about 5,9875 USD with an average of about 1,100 USD. 
...
kl2 <- kiva_loan1%>%filter(funded_amount>0)
# create data frame
...
grid.arrange(p.k1,p.k2,p.k3,p.k4, nrow=2, bottom="Region",left="Frequency", top=textGrob("Top 5 Funded Regions per Year",gp=gpar(fontsize=15,font=10)))
```

From the bar plot, there is a mix of top regions that are top funded per year. Is there a relationship between funded location and poverity index?
...
mpi.world_region.d3$label <- paste(mpi.world_region.d3$world_region, mpi.world_region.d3$sumloan_amount.region, sep=", ")

#size is total loan
treemap(mpi.world_region.d3, index="label",vSize="sumloan_amount.region",vColor="MPI_region",title="Loans by World Region (Total in $M)",title.legend = "Region MPI (mean)", type="value",range=c(0.30,0.05),position.legend= "right", palette=brewer.pal(n=5,"RdYlGn"))
...
From the treemap, Sub-Saharan Africa is the poorest World Region and has received a large portion of the total Kiva loans. 
Latin America and Caribbean are lower on the poverty index, but receive the second largest portion of Kiva loans. 
While South Asia, high on the poverty index, receives the second smallest portion of Kiva loans. Will focus on these 3 regions and dig deeper into these differences and what other factors are playing into loan distribution.

```{r, echo=FALSE, fig.height=5,fig.width=12,message=FALSE,warning=FALSE}
# countries within Sub-Saharan Africa
mpi.country.Africa <- filter(mpi.loan.country.region.d, world_region=="Sub-Saharan Africa")
mpi.country.Africa$label <- paste(mpi.country.Africa$country, mpi.country.Africa$sumloan_amount, sep=", ")
treemap(mpi.country.Africa, index="label",vSize="sumloan_amount",vColor="MPI_country",title="Loans by Country in Sub-Saharan Africa (Total in $M)",title.legend = "Country MPI (mean)", type="value",range=c(0.50,0.20),position.legend= "right", palette=brewer.pal(n=5,"RdYlGn"))
...
mpi.country.LA$label <- paste(mpi.country.LA$country, mpi.country.LA$sumloan_amount, sep=", ")
treemap(mpi.country.LA, index="label",vSize="sumloan_amount",vColor="MPI_country",title="Loans by Country in Latin America and Caribbean (Total in $M)",title.legend = "Country MPI (mean)", type="value",range=c(0.50,0.10),position.legend= "right", palette=brewer.pal(n=5,"RdYlGn"))

# countries within South Asia
mpi.country.SA <- filter(mpi.loan.country.region.d, world_region=="South Asia")
mpi.country.SA$label <- paste(mpi.country.SA$country, mpi.country.SA$sumloan_amount, sep=", ")
treemap(mpi.country.SA, index="label",vSize="sumloan_amount",vColor="MPI_country",title="Loans by Country in South Asia (Total in $M)",title.legend = "Country MPI (mean)", type="value",range=c(0.50,0.20),position.legend= "right", palette=brewer.pal(n=5,"RdYlGn"))
```

From these treemaps, some of the poorest countries are receiving a small portion of the total Kiva loans for the region. 
Burkina and South Sudan within Sub-Saharan Africa, Haiti within Latin America and Caribbean, and Afghanistan within South Asia. 
...
# countries with funded loans only
kl3 <- kiva_loan1%>%filter(funded_amount>0)
# create data frame
...
#size is total loan
#top.3.p <- ggplot(top.3, aes(fill=world_region,y=loan_amount.sector,x=sector))+geom_bar(stat="identity",position="fill")+guides(fill=guide_legend(title="World Region"))+theme(text=element_text(size=19))+labs(title = "", subtitle = "", y="",x="")

top.3.p1 <- ggplot(top.3.group, aes(fill=allother,y=loan_amount.sector,x=world_region))+geom_bar(stat="identity",position="fill")+guides(fill=guide_legend(title="Sector"))+theme(text=element_text(size=19),axis.text.x=element_text(angle=30, hjust=1))+labs(title = "", subtitle = "", y="",x="")
...
top.3.p2 <- ggplot(top.3.Africa1, aes(y=loan_amount.sector,x=reorder(country,-loan_amount.sector),fill=allother))+geom_bar(stat="identity")+guides(fill=guide_legend(title="Sector"))+theme(text=element_text(size=19),axis.text.x=element_text(angle=30, hjust=1))+labs(title = "", subtitle = "", y="",x="")


grid.arrange(top.3.p2, nrow=1,left=textGrob("Funded Loans",rot=90, gp=gpar(fontsize=14)),bottom=textGrob("Country", gp=gpar(fontsize=14,font=11)),top=textGrob("Relationship Between Sub-Saharan Africa and Top Sectors ", gp=gpar(fontsize=14,font=11)))
```
