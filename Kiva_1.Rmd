...
# kiva loan sector + country for logistic regression
kiva_loan.sector <- kiva_loan1%>%dplyr::select(sector,loan_amount,funded_amount,country)%>%group_by(country,sector)%>%summarise(sumloan_amount=sum(loan_amount),sumfunded_amount=sum(funded_amount))
kiva_loan.sector<- distinct(kiva_loan.sector)
...
This dataset contains $World Region$ and $MPI$ variables. $MPI$ is an index used to measure acute poverty. It is indicated at the location level in the dataset, so we will take an average for $Country$, and $World Region$.
```{r,echo=FALSE,message=FALSE}
# read poverty index for region
mpi <- read.csv("kiva_mpi_region_locations.csv", na.strings=c("","NA"))
# joining at country level and MPI at location level
mpi <- mpi%>%group_by(country)%>%mutate(MPI.c=mean(MPI))

# average MPI per world region
mpi.wr <- mpi%>%group_by(world_region)%>%mutate(MPI.wr=mean(MPI))
...
mpi.loan.join <- mpi%>%left_join(kiva_loan.join.d,by="country")%>%na.omit()

# join sector country grouping from Kiva data with MPI for logistic regression
mpi.sector.country <-mpi%>%left_join(kiva_loan.sector,by="country")%>%na.omit()
mpi.sector.country <- mpi.sector.country%>%dplyr::select(country,sector,MPI.c,sumloan_amount,sumfunded_amount)
mpi.sector.country <- distinct(mpi.sector.country)
...
#can improve by adding actual map and color MPI scale on countries
```
...
From the box plot, there is some variation in the loan amounts among the $sector$s. The Food, Housing, and Personal Use sectors have the lowest medians. This could correspond to what we saw from the treemaps. We can hypothesize that this may indicate that borrowers in these sectors are poorer (higher MPI) and need smaller loan amounts for their needs. Not necessarily true, but modeling this will give us better insight. 

So, what is the funded loan breakdown for these top sectors for World Region and Country?
...
#ggplot(kiva_loan1.gr1, aes(x=Gender, y=log.funded)) +  geom_violin()+labs(title = "", y="",x="")+stat_summary(fun.y=mean, geom="point", shape=20, size=10, color="red", fill="red") +scale_fill_brewer(palette="Set3")+guides(fill=guide_legend(title="Gender"))+theme(text=element_text(size=16))+facet_wrap(~world_region)
...
We will take a look at logistic regression for specific $sector$s of interest. Using the boxplots, we hypothesized that Food, Housing, and Personal Use loans could be an indicator of poverty (higher $MPI$). A logistic regression model may be useful in seeing this relationship. Sectors are grouped as *Not* Food, Housing, Personal Use and Food, Housing, Personal Use as binary outcomes (0 or 1).
```{r,echo=FALSE,warning=FALSE,message=FALSE}
# Food, Housing, and Personal Use Loans vs. Not
logistic.data <- mpi.sector.country%>%mutate(Sector.Var=ifelse(sector=="Food",1,ifelse(sector=="Housing",1,ifelse(sector=="Personal Use",1,0))))

# log scale
logistic.data$log.funded <- log(logistic.data$sumfunded_amount)

lm<-glm(Sector.Var ~ MPI.c+country,data = logistic.data,family = binomial)
summary(lm)

```

AIC seems a bit high. 
(Look at Multilevel Binomial Logistic Regression).
