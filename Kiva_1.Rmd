...
# if a male was involved on the loan
...
```{r, echo=FALSE, message=FALSE,warning=FALSE}
par(mfrow=c(2, 2))
# total number of loan ids in dataset
print(c(Total.Loan.Population=sum(unique(kiva_loan1$partner_id))))
# table overall gender
gender1<-as.data.frame(table(kiva_loan1$Gender.Var,kiva_loan1$year))
colnames(gender1)<-c("Gender.Var","year","Frequency")

ggplot(data=gender1, aes(x=Gender.Var, y=Frequency)) +
  geom_bar(stat="identity",fill="orange")+theme(plot.title = element_text(hjust = 0.5))+ggtitle("Frequency of Overall Gender")

# table of frequency of all female loans, all male loans, and male/female loan (having 1 male, having 2 male...)
kiva_loan1
```


```{r, echo=FALSE, message=FALSE}
#formula for female and male distributions
sum(kiva_loan1$Female.count)/(sum(kiva_loan1$Male.count)+sum(kiva_loan1$Female.count))
sum(kiva_loan1$M.F.loan) # of loans with males on a female loan
sum(kiva_loan1$M.F.loan)/sum(kiva_loan1$F.loan) # of loans with female
