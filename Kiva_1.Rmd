---
title: "Lardieri Applied Statistical Modeling Midterm Project - Kiva Loans"
output:
...
My project aims to take a deeper look at the Kiva loan data, to see if there are any underlying themes or behaviors that differ between regions and countries. Specifically, is there variation in category and amount of funding and to whom. 
...
In the dataset, an unfunded loan is funded amount=$0, a partially funded loan is funded amount<loan amount and fully funded loan is funded amount=loan amount. Over 91% of Kiva loans are fully funded!
...
From distribution, the poorest world regions are Sub-Saharan Africa and South Asia. What proportion of loans are these world regions receiving?
...
Again, these areas of focus for loan trends and what is driving these differences. One thing to consider are potential sector and activity differences between the regions/countries. Are certain types of loans more frequent? 
...
Recalling the treemap by World Region, are there similar breakdowns further by country?
...
#change order
wr$allother <- factor(wr$allother, levels = c("All Other","Retail","Food","Agriculture"))
wr<- as.data.frame(wr)
# log scale
wr$log.funded_amount.sector <- log(wr$funded_amount.sector)

ggplot(wr, aes(sector, log.funded_amount.sector,color=sector))+geom_boxplot()+guides(fill=guide_legend(title="Sector"))+theme(text=element_text(size=19),axis.text.x=element_text(angle=30, hjust=1))+labs(title = "", subtitle = "", y="",x="")+labs(title = "Distribution of Funded Loans by Sector (Log Scale)", subtitle = "", y="",x="")+theme(plot.title = element_text(hjust=0.5))
```
...
##Models
What factors play into the loans in different countries/regions
```{r}
#require(IDPmisc)
